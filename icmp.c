...
#include <json-c/json.h> // for json input
...
int main(int argc, char **argv)
{
	...
	// struct json_object *roll;

	fp = fopen("json_input/input.json", "r");
	...
	json_object_object_get_ex(parsed_json, "dst", &dst);
	// json_object_object_get_ex(parsed_json, "roll no", &roll);

	printf("source: %s\n", json_object_get_string(src));
	printf("destination: %d\n", json_object_get_int(dst));
	// printf("roll no: %d\n", json_object_get_int(roll));
	...
	saddr = inet_addr(json_object_get_string(src));
	daddr = inet_addr(json_object_get_string(dst));

	// if (argc > 3)
	// {
	payload_size = atoi("100");
	// }
